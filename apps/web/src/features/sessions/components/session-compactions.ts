...
export function assignCompactionsBeforeTurns(
	...
	const compactionsByTurn: SessionCompaction[][] = [];
	for (const _turnStartTimestamp of turnStartTimestamps) {
		compactionsByTurn.push([]);
	}
	...
	for (const compaction of compactions) {
		...
		for (
			let turnIndex = 0;
			turnIndex < turnStartTimestamps.length;
			turnIndex++
		) {
			const turnStartTimestamp = turnStartTimestamps[turnIndex];
			if (!turnStartTimestamp) {
				continue;
			}

			const turnStartTime = Date.parse(turnStartTimestamp);
			if (Number.isNaN(turnStartTime) || turnStartTime < compactionTime) {
				continue;
			}

			compactionsByTurn[turnIndex]?.push(compaction);
			break;
		}
	}
