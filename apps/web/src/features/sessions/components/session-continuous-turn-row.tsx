...
export const SessionContinuousTurnShell = memo(
	function SessionContinuousTurnShell({
		className,
		estimatedSize,
		index,
		option,
		...
		return (
			<section
				...
				className={cn(
					"grid content-start gap-3 border-b border-(--session-overview-border) px-4 py-5",
					className,
				)}
				data-continuous-turn-index={index}
				data-session-virtual-turn-shell
				style={{ minHeight: estimatedSize }}
			>
				<div className="flex min-w-0 items-center gap-2 text-xs font-medium text-(--session-overview-muted)">
					<span className="size-2 rounded-full bg-(--session-overview-accent)" />
					<span>
						{option.turnNumber === undefined
							? "Session start"
							: `Turn ${option.turnNumber}`}
				...
				<p className="line-clamp-2 text-sm text-(--session-overview-text)">
					{option.memberPreview}
				</p>
				<p className="line-clamp-2 text-sm text-(--session-overview-muted)">
					{option.preview}
				</p>
			</section>
		);
