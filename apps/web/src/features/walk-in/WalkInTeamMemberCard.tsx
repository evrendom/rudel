	...
	return (
		<li className="list-none">
			...
			>
					...
					{headerRightMetric ? (
						...
						>
							{headerRightMetric.label ? (
								<div
									className={cn(
										"text-[10px] font-semibold leading-none tracking-[0.06em] text-[#7b7671]",
										isDarkTheme ? "text-white/62" : null,
										isMutedTheme ? "text-[#d6cdc2]" : null,
									)}
								>
				...
				<div
					className="relative z-10 mt-[2px] rounded-[12px]"
					ref={statSectionRef}
					style={statSectionSurfaceStyle}
				>
					<div className="relative grid grid-cols-2 gap-[6px] [font-family:var(--dashboard-01-font-roster-mono)] text-[11px] font-normal text-[#4b4d49]">
						{statItems.map((stat) => (
							<div
								key={stat.key}
								className={cn(
