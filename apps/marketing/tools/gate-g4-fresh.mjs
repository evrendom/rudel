...
const allKeys = [...new Set([...referenceByKey.keys(), ...candidateByKey.keys()])].sort();

const authorizationFor = (state) => {
	if (state === "footer") return ["D004"];
	if (state === "auxiliary-window-dragged") return ["D001", "D002", "D003", "D005"];
	if (state.startsWith("section-")) return [];
	if (state === "aperture-mid") return [];
	if (state === "js-disabled-reference-raw") return ["reference-only raw no-JS diagnostic"];
...
for (const key of allKeys) {
	...
	if (!reference || !candidate) {
		results.push({ key, missing: { reference: !reference, candidate: !candidate } });
		continue;
	}
...
const byState = Object.values(
	...
	maximumDiffPercent: Math.max(...stateResults.map((result) => result.pixel.diffPercent)),
	meanDiffPercent: stateResults.reduce((sum, result) => sum + result.pixel.diffPercent, 0) / stateResults.length,
...
const sectionResults = comparable.filter((result) => result.state.startsWith("section-"));
const unapprovedFailures = comparable.filter((result) =>
	!result.strictPass &&
	result.sanctionedDivergences.length === 0 &&
	!result.r6Residual
);
const r6Failures = comparable.filter((result) => !result.strictPass && result.r6Residual);
const report = {
	...
	structuralTier: {
		strategy: "component structural gates + full-page paint matrix; the reference iframe tree and owned Astro DOM are intentionally not isomorphic",
		componentReports: [
	...
	sectionSummary: {
		...
		maximumDiffPercent: Math.max(...sectionResults.map((result) => result.pixel.diffPercent)),
		strictPassCount: sectionResults.filter((result) => result.strictPass).length,
	},
	strictPassed: comparable.every((result) => result.strictPass) && results.length === comparable.length,
	passed: unapprovedFailures.length === 0 && r6Failures.length === 0 && results.length === comparable.length,
