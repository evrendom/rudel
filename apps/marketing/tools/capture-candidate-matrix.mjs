...
const metadata = {
	...
	responsiveAudits: {},
	traces: {
		composedDashboardIntegration:
			".context/gates/composed/dashboard-integration/matrix.json",
		composedDashboardScrollCurve:
			".context/extractions/dashboard/composed-scroll-curve/trace.json",
		terminalWindowParity:
			".context/gates/dashboard/terminal-window-parity/after/matrix.json",
	},
};
...
const navbarStates = async (session, viewport) => {
	...
	const byText = (text) =>
		`[...document.querySelectorAll("button,a")].find((node) => node.textContent?.trim() === ${JSON.stringify(text)})`;
	...
	if (product) {
		...
		await shot({
			...
			scrollY: 0,
			scenario: ["hover Product for 350ms"],
		});
		const item = await moveTo(
			session,
			`[...document.querySelectorAll("a")].find((node) => { const rect = node.getBoundingClientRect(); return rect.width > 100 && rect.height > 30 && rect.y > 70; })`,
		);
		if (item) {
			await wait(100);
			await shot({
				...
				scrollY: 0,
				scenario: ["hover first dropdown item"],
			});
			await session.client.call("Input.dispatchMouseEvent", {
				type: "mouseMoved",
	...
	if (resources) {
		...
		await shot({
			...
			scrollY: 0,
			scenario: ["Product to Resources, 90ms"],
		});
		await wait(260);
		await shot({
			session,
...
const dashboardStates = async (session, viewport) => {
	...
	const labels = await session.evaluate(
		`[...document.querySelectorAll("[data-opaline-use-case]")].map((button) => ({ value: button.dataset.opalineUseCase, label: button.textContent.trim() }))`,
	);
	...
	const windows = await session.evaluate(
		`[...document.querySelectorAll("[data-opaline-claude-window]")].map((element, index) => { const rect = element.getBoundingClientRect(); const style = getComputedStyle(element); return { index, x: rect.x, y: rect.y, width: rect.width, height: rect.height, display: style.display, visibility: style.visibility }; })`,
	);
	metadata.responsiveAudits[viewport.name].auxiliaryWindows = {
		count: windows.length,
		visibleCount: windows.filter(
			(window) =>
	...
	await shot({
		session,
		viewport,
		state: "auxiliary-window-focus",
		scrollY: 500,
		scenario: [
			"pointer-down first visible auxiliary window",
			"verify persistent front z-order",
		],
	});
	...
	const after = await session.evaluate(
		`(() => { const element = document.querySelector("[data-opaline-claude-window]"); const rect = element?.getBoundingClientRect(); return rect ? { x: rect.x, y: rect.y } : null; })()`,
	);
	metadata.responsiveAudits[viewport.name].auxiliaryWindows.drag = {
		...
		after,
		actualDelta: after
			? { x: after.x - target.x, y: after.y - target.y }
			: null,
	};
	await shot({
		session,
		...
		scrollY: 500,
		scenario: ["drag first visible auxiliary window +120,-60"],
	});
};
...
const focusStates = async (session, viewport) => {
	await session.scrollTo(0);
	await wait(400);
	const cta = await moveTo(
		session,
		`[...document.querySelectorAll("a,button")].find((node) => node.textContent?.includes("Get Early Access"))`,
	);
	if (cta) {
		await wait(120);
		await shot({
			...
			state: "button-hover",
			scrollY: 0,
			scenario: ["hover Get Early Access"],
		});
	}
	...
	await shot({
		...
		state: "focus-visible",
		scrollY: 0,
		scenario: ["press Tab three times"],
	});
};
...
for (const viewport of viewports) {
	...
	try {
		...
		await shot({
			...
			state: "aperture-mid",
			scrollY: 0,
			scenario: ["wheel aperture to progress 0.5"],
		});
	} finally {
		await mid.close();
	}
	...
	try {
		...
		for (const state of [
			...
			await shot({
				...
				scenario: [`scroll to ${state.scrollY}`],
			});
		}
		if (viewport.width >= 992) {
			for (const state of [
				{
					name: "use-case-strip-top",
					scrollY: 0,
					scenario: ["settled composed top", "use-case strip visible"],
				},
				{
					name: "auxiliary-overhang-unclipped",
					scrollY: 0,
					scenario: ["settled composed top", "auxiliary overhang visible"],
				},
				{
					name: "canvas-show-through",
					scrollY: 300,
					scenario: ["scroll to 300", "transparent integration layers"],
				},
				{
					name: "dashboard-rise-g3b",
					scrollY: 500,
					scenario: ["scroll to 500", "motion controller ready"],
				},
				{
					name: "terminal-scroll-exit",
					scrollY: 900,
					scenario: ["scroll to 900", "terminal exits with hero"],
				},
			]) {
				await session.scrollTo(state.scrollY);
				await wait(120);
		...
		metadata.responsiveAudits[viewport.name].sections = {
			discovered: anchors.sections.length,
		...
		for (const section of anchors.sections) {
			...
			await shot({
				session,
				viewport,
				state: `section-${String(section.index + 1).padStart(2, "0")}`,
				scrollY: section.top,
				scenario: [`scroll to owned section ${section.index}`],
			});
		}
		...
		await shot({
			...
			scenario: ["scroll to document end"],
		});
	...
	const reduced = await createBrowserSession({
		url,
		...viewport,
		dpr: 1,
		reducedMotion: true,
	...
	try {
		...
		await shot({
			...
			scenario: ["reduced motion before navigation"],
		});
	} finally {
		await reduced.close();
	}

	const rawNoJs = await createBrowserSession({
		url,
		...viewport,
		dpr: 1,
	...
	try {
		...
		await shot({
			...
			scenario: ["disable scripts before navigation"],
			weakHistogram: true,
		});
	} finally {
		await rawNoJs.close();
	...
	try {
		...
		await wait(400);
		await noJs.client.call("Emulation.setScriptExecutionDisabled", {
			value: true,
		});
		await shot({
			session: noJs,
			...
			state: "js-disabled",
			scrollY: 0,
			mode: "scripts-disabled-after-settle",
			scenario: ["settle, then disable scripts"],
		});
	} finally {
		await noJs.close();
	}
...
console.log(
	JSON.stringify({ outputDirectory, shots: metadata.shots.length }, null, 2),
);
