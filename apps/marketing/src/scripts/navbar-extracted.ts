...
const header = document.querySelector<HTMLElement>(".TZTsQG_header");
...
const openMenu = (menu: MenuName) => {
	...
	if (!(viewport instanceof HTMLElement) || !previousMenu) {
		...
		delete firstViewport.querySelector<HTMLElement>(".TZTsQG_content")?.dataset
			.motion;
	...
	const previousContent =
		viewport.querySelector<HTMLElement>(".TZTsQG_content");
	if (previousContent) {
...
for (const trigger of triggers) {
	...
	trigger.addEventListener("keydown", (event) => {
		if (event.key === "ArrowDown" && menuForTrigger(trigger) === activeMenu) {
			...
			desktopPortal
				?.querySelector<HTMLAnchorElement>(".TZTsQG_content a")
				?.focus();
			return;
		}
...
const mobileTrigger = document.querySelector<HTMLButtonElement>(
	'.TZTsQG_mobileMenuTrigger[aria-haspopup="dialog"]',
);
...
document.addEventListener("click", (event) => {
	...
	if (event.target.closest(".TZTsQG_content a")) closeMenu();
