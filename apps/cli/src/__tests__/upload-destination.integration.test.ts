...
test.each(HOOK_CASES)(
	"$name refuses an unsafe RUDEL_API_BASE observably and queues the session",
	async (hookCase) => {
		const stub = startIngestStub();
		...
		try {
			...
			expect(result.exitCode).toBe(1);
			expect(result.stderr).toContain("Opaline hook upload refused");
...
test.each(HOOK_CASES)(
	"$name honors RUDEL_ALLOW_INSECURE_ENDPOINT=1",
	async (hookCase) => {
		const stub = startIngestStub();
		const fixture = await createCliFixture(hookCase.source);
		try {
...
test.each(HOOK_CASES)(
	"$name exposes and retains permanent API failures",
	async (hookCase) => {
		const message =
		...
		try {
			...
			expect(result.exitCode).toBe(0);
			expect(result.stderr).toContain("Opaline hook upload failed");
