...
describe("session_analytics computed values", () => {
	...
	test("re-ingesting the same session does not double-count tokens", async () => {
		...
		// Same transcript again, as a client retry or a re-upload would send it.
		await ingest(input);

		const rows = await readAnalytics(sessionId);

		// One logical session must read as one row with un-doubled totals.
		// ReplacingMergeTree collapses this only because both inserts share a
		// sorting key. If session_date ever differs between uploads of the same
		// session — see RUD-231's `now` fallback — this returns 2 rows and 750
		// tokens, which is exactly the failure this guards.
		expect(rows).toHaveLength(1);
		const row = rows[0];
		if (!row) throw new Error("no analytics row produced");
		expect(num(row.total_tokens)).toBe(EXPECTED.total_tokens);
	...
	test("Claude and Codex sessions both reach session_analytics", async () => {
		const claudeId = `values_src_claude_${runId}`;
		const codexId = `values_src_codex_${runId}`;
		...
		const claudeRows = await readAnalytics(claudeId);
		const codexRows = await readAnalytics(codexId);

		// Both materialized views write to the same target table. A change that
		// breaks one source silently — the RUD-221 failure mode — shows up here.
		expect(claudeRows).toHaveLength(1);
		expect(codexRows).toHaveLength(1);
		expect(claudeRows[0]?.source).toBe("claude_code");
		expect(codexRows[0]?.source).toBe("codex");
	}, 300000);
});
