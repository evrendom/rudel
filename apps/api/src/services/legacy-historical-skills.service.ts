...
export async function listLegacyHistoricalCodexSkills(
	orgId: string,
	...
	const rows = await queryClickhouse<HistoricalSkillSummaryRow>({
		...
		`,
		query_params: { orgId, skillNamePattern: CODEX_SKILL_NAME_PATTERN },
	});
	...
	return rows.map((row) => ({
		...
		codexSessionCount: Number(row.session_count),
	}));
}

export async function getLegacyHistoricalCodexSkillDetail(
	orgId: string,
	name: string,
	...
	const rows = await queryClickhouse<HistoricalSkillSessionRow>({
		query: `
			SELECT
				session_id,
				content,
				formatDateTime(session_date, '%Y-%m-%dT%H:%i:%SZ', 'UTC') AS used_at
			FROM (
				SELECT
					session_id,
	...
	const detail = buildHistoricalSkillDetail(name, rows);
	return {
		...detail,
		claudeSessionCount: 0,
		codexSessionCount: detail.sessionCount,
		sourceAgents: detail.sessionCount > 0 ? ["codex"] : [],
