...
The red hotbox marks the screen area that resolves or triggers the event. These
captures use a desktop viewport (`1440 x 1200`). For load-fired events, the
hotbox marks the surface the user must land on; for click events, it marks the
control the user acts on.

| Step | Event | Screen | User action that fires or precedes the event | Hotbox meaning |
| --- | --- | --- | --- | --- |
| 1 | `Wrapped Share Viewed` | <img src="./assets/wrapped-wom/png/01-share-viewed.png" width="320" alt="Public wrapped share view with card hotbox" /> | Open a public wrapped profile link. | The loaded public card snapshot. |
| 2 | `Wrapped Share CTA Clicked` | <img src="./assets/wrapped-wom/png/02-share-cta-clicked.png" width="320" alt="Public wrapped share CTA with Make yours hotbox" /> | Click `Make yours` from the public share. | The conversion CTA. |
| 3 | `Wrapped Referred Signup Completed` | <img src="./assets/wrapped-wom/png/03-referred-signup.png" width="320" alt="Wrapped signup screen with signup action hotbox" /> | Complete signup after entering from `share_id`. | The account creation action that produces the post-auth referred session. |
| 4 | `Wrapped Onboarding Started` | <img src="./assets/wrapped-wom/png/05-onboarding-started.png" width="320" alt="Wrapped setup screen with first setup command hotbox" /> | Land in `/wrapped` after auth/session resolution. | The setup surface that starts conversion work. |
| 5 | `Wrapped Profile Completed` | <img src="./assets/wrapped-wom/png/04-profile-completed.png" width="320" alt="Wrapped card profile screen with Continue hotbox" /> | Finish the card profile step and continue. | The profile completion CTA. |
...
| 7 | `Wrapped Activation Completed` | <img src="./assets/wrapped-wom/png/06-activation-completed.png" width="320" alt="Wrapped sessions landed screen with continue hotbox" /> | Continue after uploaded sessions are detected. | The sessions-ready continuation CTA. |
| 8 | `Wrapped Story Started` | <img src="./assets/wrapped-wom/png/07-story-started.png" width="320" alt="Wrapped story start screen with story stage hotbox" /> | Enter the wrapped story after activation. | The story stage surface on mount. |
| 9 | `UI Utility Used: wrappedStageViewed` | <img src="./assets/wrapped-wom/png/07-story-started.png" width="320" alt="Wrapped story viewed screen with stage hotbox" /> | Same story entry as step 8. | Diagnostic stage-view surface. |
| 10 | `UI Utility Used: wrappedSharePreviewOpened` | <img src="./assets/wrapped-wom/png/08-share-preview-opened.png" width="320" alt="Wrapped reveal screen with share preview continue hotbox" /> | Continue from the final reveal into the share preview. | The reveal footer action. |
| 11 | `Wrapped Share Action Triggered` | <img src="./assets/wrapped-wom/png/09-share-action-triggered.png" width="320" alt="Wrapped share stage with Share on X hotbox" /> | Click `Share on X`, copy image, or download PNG. | A distribution action; screenshot shows X share. |
| 12 | `Wrapped Share Created` | <img src="./assets/wrapped-wom/png/09-share-action-triggered.png" width="320" alt="Wrapped share stage with share action hotbox" /> | Complete a share action that needs a public URL. | Same share action, but this event fires once a persistent child share exists. |
| 13 | `Dashboard Navigation Clicked: wrappedContinueToDashboard` | <img src="./assets/wrapped-wom/png/10-dashboard-exit.png" width="320" alt="Wrapped share stage with dashboard continue hotbox" /> | Click `Continue to dashboard`. | The wrapped exit CTA. |
